...
router.get(
	...
	asyncHandler(async (req, res, next) => {
		try {
			...
			res.locals.nextHalvingData = utils.nextHalvingEstimates(res.locals.difficultyPeriodFirstBlockHeader, res.locals.latestBlocks[0], res.locals.difficultyAdjustmentData);
						...
						async () => {
							...
							if (addressDetails) {
								...
								if (addressDetails.txids) {
									...
									const rawTxResult = await (global.txindexAvailable ? coreApi.getRawTransactionsWithInputs(txids, 5) : coreApi.getRawTransactionsByHeights(txids, blockHeightsByTxid).then((transactions) => ({ transactions, txInputsByTransaction: {} })));
...
router.get("/quotes", function (req, res, next) {
	...
	listNewFirst = listNewFirst.filter((x) => {
		return !x.duplicateIndex;
	});
